# Recreate Necklace Try-On Handoff

This document describes the standalone WebAR necklace rebuild under `recreate/`. It is intended for migrating this implementation into a clean repository while preserving the architecture, tuning decisions, and lessons learned.

## Current Status

The `recreate` demo is a standalone browser page for necklace virtual try-on using:
...
```text
recreate/index.html
recreate/recreate-necklace.js
recreate/necklace-catalog.json
recreate/custom/DN-0075-1-removebg.png
recreate/custom/DN-0105-1_removebg.png
recreate/custom/DN-0075-3D.glb
recreate/custom/DN-0105-3d.glb
```
...
```bash
node --check recreate/recreate-necklace.js
node -e "JSON.parse(require('fs').readFileSync('recreate/necklace-catalog.json','utf8')); console.log('catalog ok')"
```
...
`recreate/necklace-catalog.json` currently contains:
...
Built `recreate/index.html` with:
...
recreate/necklace-catalog.json
recreate/custom/*
recreate/assets/envmaps/venice_sunset_512.hdr
recreate/neuralNets/NN_NECKLACE_9.json
recreate/libs/three/v136/*
recreate/dist/WebARRocksFace.js
recreate/helpers/WebARRocksFaceThreeHelper.js
recreate/helpers/landmarksStabilizers/OneEuroLMStabilizer.js
```
...
1. Open `recreate/index.html` through a local server.
